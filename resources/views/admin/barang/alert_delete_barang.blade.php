...
<div class="flex items-center justify-center min-h-screen backdrop-blur-sm bg-black/30 py-4">
    <div class="w-full max-w-md bg-white p-6 rounded-lg  border border-white">
        <label id="btnDeleteBarangClose" class="float-right bg-blue-50 px-2 rounded-full">X</label>

        <p>Apakah anda yakin ingin menghapus
            "<label id="namaBarang" class="uppercase font-bold"></label>"?
        </p>
        <form action="{{ route('barang.delete') }}" method="POST">
            @csrf
            @method('DELETE')
            <div class="mb-4">
                <input type="text" class="hidden" name="id_barangDelete" id="id_barangDelete" class="border border-gray-300 focus:outline-none focus:border-blue-500 rounded px-3 py-2 w-full">
            </div>
