...
<div class="flex items-center justify-center min-h-screen backdrop-blur-sm bg-black/30 py-4">
    <div class="w-full max-w-md bg-white p-6 rounded-lg  border border-white">
        <label id="btnDeleteKaryawanClose" class="float-right bg-blue-50 px-2 rounded-full">X</label>

        <p>Apakah anda yakin ingin menghapus
            "<label id="namaKaryawan" class="uppercase font-bold"></label> <label id="id-karyawan" class="uppercase font-bold"></label>"?
        </p>
        <form action="{{ route('karyawan.delete') }}" method="POST">
            @csrf
            @method('DELETE')
            <div class="mb-4">
                <input type="text" class="hidden" name="id_karyawanDelete" id="id_karyawanDelete" class="border border-gray-300 focus:outline-none focus:border-blue-500 rounded px-3 py-2 w-full">
            </div>
